...
impl<'g, SPO: 'g, OPS: 'g>
    Into<graph::BlankNodeOrIRI<'g, BlankNodePtr<'g, SPO, OPS>, IRIPtr<'g, SPO, OPS>>>
    for BlankNodePtr<'g, SPO, OPS>
where
    SPO: CompactTriple<u32>,
    OPS: CompactTriple<u32>,
{
    ...
    OPS: CompactTriple<u32>,
{
    ...
    fn language(&self) -> Option<&str> {
        self.language
            .map(|l| self.graph.datatype_or_lang.get(StringId { id: l }))
    }
}
...
pub type BlankNodeOrIRI<'t, SPO, OPS> =
    graph::BlankNodeOrIRI<'t, BlankNodePtr<'t, SPO, OPS>, IRIPtr<'t, SPO, OPS>>;
pub type Resource<'t, SPO, OPS> =
    graph::Resource<'t, BlankNodePtr<'t, SPO, OPS>, IRIPtr<'t, SPO, OPS>, LiteralPtr<'t, SPO, OPS>>;
...
impl<'g, SPO: 'g, OPS: 'g, T>
    graph::Triple<'g, BlankNodePtr<'g, SPO, OPS>, IRIPtr<'g, SPO, OPS>, LiteralPtr<'g, SPO, OPS>>
    for Triple<'g, SPO, OPS, T>
where
    SPO: CompactTriple<u32>,
    OPS: CompactTriple<u32>,
    T: CompactTriple<u32>,
    ...
    fn subject(&self) -> BlankNodeOrIRI<'g, SPO, OPS> {
        if self.triple.subject_is_iri() {
            graph::BlankNodeOrIRI::IRI(IRIPtr {
                graph: self.graph,
                iri: self.triple.subject(),
            })
        } else {
            graph::BlankNodeOrIRI::BlankNode(
                BlankNodePtr {
                    graph_id: self.graph.graph_id,
                    node_id: self.triple.subject(),
                    phantom: PhantomData,
                },
                PhantomData,
            )
    ...
    fn object(&self) -> Resource<'g, SPO, OPS> {
        if self.triple.object_is_iri() {
            graph::Resource::IRI(IRIPtr {
                graph: self.graph,
                iri: self.triple.object(),
            })
        } else if self.triple.object_is_blank_node() {
            graph::Resource::BlankNode(
                BlankNodePtr {
                    graph_id: self.graph.graph_id,
                    node_id: self.triple.object(),
                    phantom: PhantomData,
                },
                PhantomData,
            )
